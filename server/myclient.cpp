...
void MyClient::readyRead()
{
    ...
    } else if (str.startsWith("getres"))
    {
        ...
        {
            QSqlQuery q3;
            q3.prepare("SELECT ship_body FROM ship WHERE player_id=:player_id");
            q3.bindValue(":player_id", _player->_player_id);
            q3.exec();
            int ship_body = 0;
            if (q3.next())
            {
                ship_body = q3.value(0).toInt();
            }
            if (ship_body == 0)
            {
                qDebug() << "Error: ship_body corrupted.";
                exit(-1);
            }
            QSqlQuery q4;
            q4.prepare("SELECT class FROM ship_body WHERE body_id=:ship_body");
            q4.bindValue(":ship_body", ship_body);
            q4.exec();
            QString class1;
            if (q4.next())
            {
                class1 = q4.value(0).toString();
            }
            data[pos] = class1[0];
            data[_player->_pos] = ' ';
            _player->_pos = pos;
            QSqlQuery q5;
            q5.prepare("UPDATE map SET data=:data WHERE map_id=:map_id");
            q5.bindValue(":map_id", _player->_map);
            q5.bindValue(":data", data);
            q5.exec();
            QSqlQuery q6;
            q6.prepare("SELECT fuel FROM ship_point WHERE ship_id=:ship_id");
            q6.bindValue(":ship_id", ship_body);
            q6.exec();
            int fuel = 0;
            if (q6.next())
            {
                fuel = q6.value(0).toInt();
            }
            fuel++;
            QSqlQuery q7;
            q7.prepare("UPDATE ship_point SET fuel=:fuel WHERE ship_id=:ship_id");
            q7.bindValue(":ship_id", ship_body);
            q7.bindValue(":fuel", fuel);
            q7.exec();
            QSqlQuery q2;
            q2.prepare("UPDATE player SET pos=:pos WHERE player_id=:player_id");
            q2.bindValue(":player_id", _player->_player_id);
            q2.bindValue(":pos", pos);
            q2.exec();
            SendParams();
        }
...
void MyClient::SendParams()
{
    ...
    QSqlQuery q1;
    q1.prepare("SELECT life, energy, armor, fuel, net_speed, cartograph_link, grab_points, "
               "radar_ships, scaner_predm, fire, fire_speed, fire_link FROM ship_point WHERE "
               "ship_id=:ship");
    q1.bindValue(":ship", ship);
    q1.exec();
    int life = 0, energy, armor, fuel, net_speed, cartograph_link, grab_points,
            radar_ships, scaner_predm, fire, fire_speed, fire_link;
    if (q1.next())
    {
        life = q1.value(0).toInt();
        energy = q1.value(1).toInt();
        armor = q1.value(2).toInt();
        fuel = q1.value(3).toInt();
        net_speed = q1.value(4).toInt();
        cartograph_link = q1.value(5).toInt();
        grab_points = q1.value(6).toInt();
        radar_ships = q1.value(7).toInt();
        scaner_predm = q1.value(8).toInt();
        fire = q1.value(8).toInt();
        fire_speed = q1.value(9).toInt();
        fire_link = q1.value(10).toInt();
    }
    if (life == 0)
    {
    ...
    obj["armor"] = armor;
    obj["fuel"] = fuel;
    obj["net_speed"] = net_speed;
    obj["cartograph_link"] = cartograph_link;
    obj["grab_points"] = grab_points;
    obj["radar_ships"] = radar_ships;
    obj["scaner_predm"] = scaner_predm;
    obj["fire"] = fire;
    obj["fire_speed"] = fire_speed;
    obj["fire_link"] = fire_link;
    QJsonDocument doc(obj);
